...
type Product struct {
	...
	Orders_id int `db:"order_id_items" json:",omitempty"`
}
...
func GetProductsByOrderId(w http.ResponseWriter, r *http.Request) {
	...
	err = data.Select(&p, `SELECT o.order_items_id,
							p.product_name, p.gender, p.size, p.color, p.price, p.descount_price
							FROM order_items o
