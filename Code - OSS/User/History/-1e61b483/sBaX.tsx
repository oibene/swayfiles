...
export default function ProductComponents(data : ProductComponentsProps) {
    ...
    if (desc.indexOf('.') !== -1){
        first_part = desc.slice((title != null) ? desc.indexOf('-') : 0 , desc.indexOf('.'))

    }
        

        second_title = desc.slice(desc.indexOf('.') + 1, desc.length)


    if (desc.indexOf(':') !== -1)
        second_title = desc.slice(desc.indexOf('.')+1, desc.indexOf(":"))
